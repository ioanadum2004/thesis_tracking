...
Optionally produces a second plot in the mapped coordinate space from the
TrainML paper, where hits from the same helix cluster together:
  r1 = sqrt(x^2 + y^2 + z^2)   (3D radius)
  x2 = x / r1
  y2 = y / r1
  r2 = sqrt(x^2 + y^2)          (2D radius)
  z2 = z / r2
...
def main():
    ...
    p.add_argument('--mapped', dest='mapped', action='store_true', default=False,
                   help='also produce a mapped-coordinate plot (<out_prefix>_mapped.html/.png) '
                        'using the TrainML helix transformation: x2=x/r1, y2=y/r1, z2=z/r2 '
                        'where r1=sqrt(x^2+y^2+z^2) and r2=sqrt(x^2+y^2)')
    p.add_argument('--mapped-only', dest='mapped_only', action='store_true', default=False,
                   help='produce only the mapped-coordinate plot, skip the raw-coordinate plot')
    ...
    # p = argparse.ArgumentParser(description='Create 2D image of spacepoints from hits.root')
    # p.add_argument('out_prefix', help='output prefix (image written as <prefix>.png)')
    # p.add_argument('--hits', '-i', help='path to hits.root (default: ./hits.root)', default='hits.root')
    # p.add_argument('--dpi', type=int, default=150, help='image DPI (default: 150)')
    # p.add_argument('--interactive', dest='interactive', action='store_true', help='write an interactive HTML view using plotly (<out_prefix>.html)')
    ...
    # p.add_argument('--mapped', dest='mapped', action='store_true', default=False,
    #                  help='also produce a mapped-coordinate plot (<out_prefix>_mapped.html/.png) '
    #                         'using the TrainML helix transformation: x2=x/r1, y2=y/r1'
    #                         'where r1=sqrt(x^2+y^2+z^2)')
    # p.add_argument('--mapped-only', dest='mapped_only', action='store_true', default=False,
    #                     help='produce only the mapped-coordinate plot, skip the raw-coordinate plot')
    # p.set_defaults(interactive=True, static=False)
    ...
    if args.static and not args.mapped_only:
        try:
    ...
    if args.interactive and not args.mapped_only:
        try:
            import warnings
    ...
    # ------------------------------------------------------------------ #
    # Mapped coordinate plot (TrainML helix transformation)               #
    # x2 = x/r1,  y2 = y/r1,  z2 = z/r2                                  #
    # where r1 = sqrt(x^2+y^2+z^2),  r2 = sqrt(x^2+y^2)                  #
    # ------------------------------------------------------------------ #
    if args.mapped or args.mapped_only:
        r1 = np.sqrt(x**2 + y**2 + z**2)
        # Avoid division by zero (degenerate hits at origin)
        safe_r1 = np.where(r1 == 0, 1.0, r1)
        safe_r2 = np.where(r == 0, 1.0, r)

        x2 = x / safe_r1
        y2 = y / safe_r1
        z2 = z / safe_r2

        mapped_title = f"Mapped spacepoints ({len(x2)} hits)"
        if pids is not None:
            unique_particles = len(np.unique(pids[pids > 0]))
            mapped_title += f" — {unique_particles} unique particles"

        # Static PNG
        if args.static:
            out_mapped_png = Path(f"{out_prefix}_mapped.png")
            try:
                plot_spacepoints(x2, y2, z2, labels=labels, color_map=color_map,
                                 out_path=out_mapped_png, title=mapped_title, dpi=args.dpi)
            except Exception as e:
                print(f"Error creating mapped plot: {e}")
                sys.exit(1)
            print(f"Wrote mapped image: {out_mapped_png}")
        ...
        if args.interactive:
            ...
            if npoints > max_points:
                idx = np.random.default_rng(seed=0).choice(npoints, size=max_points, replace=False)
                xs2 = x2[idx]; ys2 = y2[idx]; zs2 = z2[idx]
                labs2 = [labels[i] for i in idx]
                pids2 = pids[idx] if pids is not None else None
                info_msg2 = f"(downsampled from {npoints} to {max_points} points)"
            else:
                xs2 = x2; ys2 = y2; zs2 = z2
                labs2 = labels
                pids2 = pids
                info_msg2 = ""
            ...
            for i in range(len(xs2)):
                hi = (f"x2: {xs2[i]:.4f}<br>y2: {ys2[i]:.4f}<br>z2: {zs2[i]:.4f}<br>"
                      f"Volume: {labs2[i]}<br>")
                if pids2 is not None:
                    pid_val = int(pids2[i])
                    if pid_val != 0:
                        hi += f"particle_id: {pid_val}"
                        if pid_val in particle_info:
                            pt, eta = particle_info[pid_val]
                            hi += f" [Pt={pt:.3f} GeV, η={eta:.3f}]" if eta is not None else f" [Pt={pt:.3f} GeV]"
                        hi += "<br>"
                    else:
                        hi += "particle_id: noise<br>"
                hover_text2.append(hi)

            fig2 = go.Figure()
            colors = px.colors.qualitative.Set3

            if pids2 is not None:
                unique_particles2 = sorted(np.unique(pids2))
                for i, pid_val in enumerate(unique_particles2):
                    mask_pid = (pids2 == pid_val)
                    name = 'Noise' if pid_val == 0 else (
                        f"Particle {int(pid_val)} [Pt={particle_info[int(pid_val)][0]:.3f} GeV"
                        + (f", η={particle_info[int(pid_val)][1]:.3f}]" if particle_info.get(int(pid_val), (None, None))[1] is not None else "]")
                        if int(pid_val) in particle_info else f"Particle {int(pid_val)}"
                    )
                    color = 'gray' if pid_val == 0 else colors[i % len(colors)]
                    fig2.add_trace(go.Scatter3d(
                        x=xs2[mask_pid], y=ys2[mask_pid], z=zs2[mask_pid],
                        mode='markers',
                        marker=dict(size=3, color=color, opacity=0.8),
                        text=[hover_text2[j] for j in range(len(xs2)) if mask_pid[j]],
                        hoverinfo='text',
                        name=name,
                        showlegend=True
                    ))
            else:
                unique_labs2 = list(dict.fromkeys(labs2))
                for lab in unique_labs2:
                    mask_lab = [l == lab for l in labs2]
                    fig2.add_trace(go.Scatter3d(
                        x=xs2[mask_lab], y=ys2[mask_lab], z=zs2[mask_lab],
                        mode='markers',
                        marker=dict(size=3, color=color_map.get(lab, 'gray'), opacity=0.8),
                        text=[hover_text2[j] for j in range(len(xs2)) if mask_lab[j]],
                        hoverinfo='text',
                        name=lab,
                        showlegend=True
                    ))
            ...
            fig2.update_layout(
                title=dict(text=f"{mapped_title} {info_msg2}", x=0.5, xanchor='center'),
                scene=dict(
                    xaxis=dict(title='x/r₁', backgroundcolor="white", gridcolor="lightgray"),
                    yaxis=dict(title='y/r₁', backgroundcolor="white", gridcolor="lightgray"),
                    zaxis=dict(title='z/r₂', backgroundcolor="white", gridcolor="lightgray"),
                    aspectmode='data'
                ),
                showlegend=True,
            ...
            out_mapped_html = Path(f"{out_prefix}_mapped.html")
            try:
                fig2.write_html(str(out_mapped_html), include_plotlyjs='cdn')
            except Exception as e:
                print(f"Error writing mapped interactive HTML: {e}")
                sys.exit(1)

            print(f"Wrote mapped interactive HTML: {out_mapped_html}")
