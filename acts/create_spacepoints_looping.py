...
Overlaid on both panels:
  ...
  - Beam axis  (z-axis on the left panel as a dot at origin;
...
DEFAULT_MAX_PT = 0.5   # GeV — tracks below this get connected lines
...
def read_particle_pt(hits_path):
    ...
    try:
        ...
        pid_arr = t[pid_branch].array()
        pt_arr  = t[pt_branch].array()

        # uproot can return jagged (awkward) arrays for multi-event files.
        # Flatten unconditionally using several fallback strategies.
        def flatten(arr):
            # awkward array — try .to_numpy() or .flatten()
            ...
            if a.dtype == object or a.ndim > 1:
                return np.concatenate([np.atleast_1d(np.asarray(x)) for x in a])
            return a

        pid_arr = flatten(pid_arr)
        pt_arr  = flatten(pt_arr)

        return {int(p): float(pt) for p, pt in zip(pid_arr, pt_arr)}
...
def infer_detector_layers(r, z, n_bins=800, prominence_fraction=0.02):
    ...
    Returns
    -------
    layer_radii : sorted array of r values at barrel layer centres
    layer_z     : sorted array of |z| values at endcap disc centres
    """
    # --- barrel layers: peaks in r histogram ---
    r_counts, r_edges = np.histogram(r, bins=n_bins)
    r_centres = 0.5 * (r_edges[:-1] + r_edges[1:])
    r_threshold = prominence_fraction * r_counts.max()
    layer_radii = _find_peaks(r_centres, r_counts, r_threshold, min_separation_mm=5.0)

    # --- endcap discs: peaks in |z| histogram (exclude |z| < 50 mm) ---
    az = np.abs(z)
    az = az[az > 50.0]
    if len(az) > 0:
        ...
        layer_z = _find_peaks(z_centres, z_counts, z_threshold, min_separation_mm=10.0)
    else:
        layer_z = np.array([])
...
def _find_peaks(centres, counts, threshold, min_separation_mm=5.0):
    """Simple local-maximum peak finder."""
    peaks = []
    n = len(counts)
    for i in range(1, n - 1):
        if counts[i] > threshold and counts[i] >= counts[i-1] and counts[i] >= counts[i+1]:
            if not peaks or (centres[i] - peaks[-1]) > min_separation_mm:
...
def make_particle_colormap(unique_pids):
    """Return {pid: rgba} using tab20, gray for noise."""
    cmap = plt.get_cmap('tab20')
    out = {0: (0.6, 0.6, 0.6, 0.35)}   # noise: gray, very transparent
...
def draw_detector_overlay_xy(ax, layer_radii, r_max):
    """Draw barrel layer circles on the x-y panel."""
...
def draw_detector_overlay_rz(ax, layer_radii, layer_z, r_max, z_max):
    ...
    # Beam axis: horizontal line at r=0
...
def make_figure(x, y, z, r, pids, pt_lookup, max_pt, layer_radii, layer_z,
    ...
    fig, (ax_xy, ax_rz) = plt.subplots(1, 2, figsize=(14, 6))

    # ------------------------------------------------------------------
    # Draw detector geometry first (behind hits)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Separate particles into high-pT (dots) and low-pT (lines + dots)
    # ------------------------------------------------------------------
    ...
    if pids is not None:
        ...
        # No particle info — just plot everything as gray dots
        pid_to_idx = {0: list(range(len(x)))}

    low_pt_legend_added  = False
    high_pt_legend_added = False
    noise_legend_added   = False

    for pid_val, idxs in pid_to_idx.items():
        idxs = np.array(idxs)
        ...
        # Noise
        if pid_val == 0:
            kw = dict(c=[col]*len(idxs), s=4, alpha=0.3,
        ...
        if is_low_pt:
            # Order hits by r so lines follow the helix outward (and back)
            order = np.argsort(hr)
            ox = hx[order]; oy = hy[order]
            or_ = hr[order]; oz = hz[order]

            lw = 1.0
            ms = 18   # marker size squared
            zo = 4

            ax_xy.plot(ox, oy, '-', color=col, linewidth=lw,
                       alpha=0.9, zorder=zo)
            ax_xy.scatter(ox, oy, c=[col]*len(ox), s=ms,
                          linewidths=0, zorder=zo+1)

            ax_rz.plot(oz, or_, '-', color=col, linewidth=lw,
                       alpha=0.9, zorder=zo)
            ax_rz.scatter(oz, or_, c=[col]*len(oz), s=ms,
                          linewidths=0, zorder=zo+1)

            if not low_pt_legend_added:
                low_pt_legend_added = True
        else:
            # High-pT or unknown pT: dots only
            kw = dict(c=[col]*len(idxs), s=10, alpha=0.55,
                      linewidths=0, zorder=3)
            ax_xy.scatter(hx, hy, **kw)
            ax_rz.scatter(hz, hr, **kw)
            if not high_pt_legend_added:
                high_pt_legend_added = True
    ...
    ax_xy.set_title('Transverse plane  (x – y)', fontsize=12)
    ...
    ax_rz.set_title('Longitudinal plane  (z – r)', fontsize=12)
    ...
    if high_pt_legend_added:
        label = f'High-pT track  (pT ≥ {max_pt} GeV)' if pt_lookup else 'Track hit'
        legend_handles.append(
            mlines.Line2D([], [], marker='o', linestyle='None',
                          color='steelblue', markersize=5, alpha=0.6,
                          label=label))

    if low_pt_legend_added:
        legend_handles.append(
            mlines.Line2D([], [], marker='o', linestyle='-',
                          color='tomato', markersize=5,
                          label=f'Low-pT track  (pT < {max_pt} GeV) — connected by r'))
    ...
    if not pt_lookup:
        legend_handles.append(
            mpatches.Patch(color='none',
                           label='(particles.root not found — pT unknown)'))
...
def main():
    ...
    if pt_lookup:
        n_low = sum(1 for p, pt in pt_lookup.items() if pt < args.max_pt)
        title += f"  —  {n_low} low-pT tracks (< {args.max_pt} GeV)"
