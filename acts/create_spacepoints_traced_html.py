...
def add_detector_layers(fig, opacity=0.08):
    ...
    barrel_layers = [
        ...
        ('SStrip B L2', 500,  1100, 'tomato'),
        ('SStrip B L3', 660,  1100, 'tomato'),
        ('LStrip B L0', 820,  1100, 'seagreen'),
        ('LStrip B L1', 1020, 1100, 'seagreen'),
    ]

    # --- Endcap disks: (label, r_inner_mm, r_outer_mm, z_positions_mm, color) ---
    endcap_layers = [
        # CHANGE THESE TO MATCH YOUR GEOMETRY - 
        ('Pixel EC',   0,  175, [-1500, -1300, -1100, -960, -820, -700, -600, 600, 700, 820, 960, 1100, 1300, 1500], 'royalblue'),
        ('Strip EC', 200,  1020, [-2950, -2550, -2150, -1800, -1500, -1220, 1220, 1500, 1800, 2150, 2550, 2950], 'tomato'),
    ]
...
def add_detector_layers_from_hits(fig, x, y, z, vol, color_map=None, opacity=0.08):
    """
    Infer detector layer geometry directly from hit positions.
    Groups hits by volume_id, computes the median r and z-extent per layer.
    """
    import numpy as np
    import plotly.graph_objects as go
    ...
    unique_vols = np.unique(vol)
    for vid in unique_vols:
        vmask = (vol == vid)
        vr = r[vmask]
        vz = z[vmask]

        label = vol_label(vid)   # reuse your existing vol_label function
        color = color_map.get(label, 'gray')

        median_r = np.median(vr)
        z_min, z_max = np.min(vz), np.max(vz)
        r_min, r_max = np.min(vr), np.max(vr)

        show_legend = label not in seen_labels
        seen_labels.add(label)

        # Decide barrel vs endcap by aspect ratio:
        # if the layer is thin in r but extended in z → barrel (cylinder)
        # if thin in z but extended in r → endcap (disk)
        r_spread = r_max - r_min
        z_spread = z_max - z_min

        if z_spread > r_spread:
            # Barrel: draw a cylinder at median_r
            ...
            # Endcap: draw an annular disk at median z
            median_z = np.median(vz)
            r_vals = np.linspace(r_min, r_max, 20)
            T, R = np.meshgrid(theta, r_vals)
            X = R * np.cos(T)
            Y = R * np.sin(T)
            Z = np.full_like(X, median_z)
            fig.add_trace(go.Surface(
                x=X, y=Y, z=Z,
                colorscale=[[0, color], [1, color]],
                showscale=False,
...
def main():
    ...
    if args.interactive:
        ...
        if pids_sampled is not None:
            ...
            for i, pid_val in enumerate(unique_particles):
                ...
                if pid_val == 0:
                    name = 'Noise'
                    color = 'gray'
                    mode = 'markers'          # noise: dots only, no lines
                else:

                    # --- option 1 ---

                    # # Pass 1: rough ordering by r
                    # r_sort = np.argsort(pid_r)
                    # pid_x, pid_y, pid_z = pid_x[r_sort], pid_y[r_sort], pid_z[r_sort]
                    # pid_r = pid_r[r_sort]
                    # pid_hover = [pid_hover[k] for k in r_sort]

                    # # Pass 2: arc length re-sort
                    # pts = np.stack([pid_x, pid_y, pid_z], axis=1)
                    # diffs = np.diff(pts, axis=0)
                    # seg_lengths = np.linalg.norm(diffs, axis=1)
                    # arc = np.concatenate([[0], np.cumsum(seg_lengths)])
                    # arc_sort = np.argsort(arc)
                    # pid_x, pid_y, pid_z = pid_x[arc_sort], pid_y[arc_sort], pid_z[arc_sort]
                    # pid_hover = [pid_hover[k] for k in arc_sort]

                    # --- option 2 ---

                    cx = np.mean(pid_x)
                    cy = np.mean(pid_y)
                    angles = np.arctan2(pid_y - cy, pid_x - cx)
                    angles_unwrapped = np.unwrap(angles)
                    sort_idx = np.argsort(angles_unwrapped)
                    pid_x = pid_x[sort_idx]
                    pid_y = pid_y[sort_idx]
                    pid_z = pid_z[sort_idx]
                    pid_hover = [pid_hover[k] for k in sort_idx]

                    # ---------

                    if pid_val in particle_info:
                        pt, eta = particle_info[int(pid_val)]
                        name = f"Particle {int(pid_val)} [Pt={pt:.3f} GeV, η={eta:.3f}]" if eta is not None else f"Particle {int(pid_val)} [Pt={pt:.3f} GeV]"
                    else:
                        name = f"Particle {int(pid_val)}"
                    color = colors[i % len(colors)]
                    mode = 'lines+markers'    # real particles: connect hits

                # Sort hits along the trajectory by radial distance from beam axis.
                # For looping tracks this is approximate but still much better than
                # random ordering.
                sort_idx = np.argsort(pid_r)
                pid_x = pid_x[sort_idx]
                pid_y = pid_y[sort_idx]
                pid_z = pid_z[sort_idx]
                pid_hover = [pid_hover[k] for k in sort_idx]
            ...
            for lab in unique_labels:
                ...
                fig.add_trace(go.Scatter3d(
                    ...
                    z=lab_z,
                    mode='markers',
                    marker=dict(size=3, color=plotly_color_map[lab], opacity=0.2),
                    text=lab_hover,
                    hoverinfo='text',
                    name=lab,
                    showlegend=True
        ...
        # Update layout
        fig = add_detector_layers(fig, opacity=0.15)
        # fig = add_detector_layers_from_hits(fig, x, y, z, v, color_map=color_map, opacity=0.08)
        fig.update_layout(
            title=dict(
