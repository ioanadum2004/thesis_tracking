...
Configuration:
    acorn_configs/track_building.yaml (or specify with --config)
"""
...
Examples:
  ...
  python build_tracks.py --config acorn_configs/track_building.yaml
...
Note: Make sure to run infer_gnn.py first to generate edge_scores on graphs.
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Path to track building config file (default: acorn_configs/track_building.yaml)'
    )
    args = parser.parse_args()
    
    # Default config path
    if args.config is None:
        config_file = SCRIPT_DIR / 'acorn_configs' / 'track_building.yaml'
    else:
