#!/usr/bin/env python3
"""
MLP Segment Matching Track Builder

Builds tracks using helix-based segment matching, where segment pairs are scored
by a trained MLP instead of the hand-crafted Gaussian-weighted heuristic used in
segment_matching_track_builder.py.
...
Algorithm:
    ...
    3. Score all candidate segment pairs with a trained MLP
    4. Greedy matching: assign highest-scoring pairs first
    5. Evaluate and plot

Usage:
    python MLP_segment_matching_track_builder.py testset
    python MLP_segment_matching_track_builder.py testset --use-gt-segments
    python MLP_segment_matching_track_builder.py testset --score-cut 0.8
    python MLP_segment_matching_track_builder.py testset --skip-build
"""
...
from low_pt_custom_utils.mlp_segment_matching import (
    match_segments_mlp,
    load_mlp_model,
)
...
def build_tracks_for_event_mlp(graph, config, model, device):
    """
    Build tracks for one event using MLP-scored helix segment matching.
    ...
    # Step 3: MLP-based matching
    matched_tracks, unmatched = match_segments_mlp(
        segments, model, mlp_config, device=device,
        feature_scales=getattr(model, "feature_scales", None),
    )
    ...
    # Statistics
    outer_r_threshold = mlp_config.get("outer_r_threshold", 1000.0)
...
def run_mlp_segment_matching(dataset_name, config, model, device,
                              score_cut=None, use_gt_segments=None):
    """Build tracks for all events in a dataset using MLP-scored segment matching."""
    ...
    print(f"MLP Segment Matching Track Builder")
    ...
    print(f"  MLP threshold:      {mlp_config.get('mlp_score_threshold', 0.5)}")
    ...
    for event_path in tqdm(input_paths, desc=f"Building tracks for {dataset_name}"):
        ...
        labels, event_stats = build_tracks_for_event_mlp(graph, config, model, device)
    ...
    print(f"\n{'='*70}")
    print(f"MLP SEGMENT MATCHING SUMMARY - {dataset_name.upper()}")
    print(f"{'='*70}")
    ...
    print(f"  Standalone (no match): {total_stats['n_no_match']:6d}  (no MLP match found)")
...
def main():
    parser = argparse.ArgumentParser(
        description="MLP helix segment matching track builder",
...
Examples:
  ...
  python MLP_segment_matching_track_builder.py testset --skip-build
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to config file (default: acorn_configs/track_building_stage_(3)/mlp_segment_matching.yaml)",
    )
    parser.add_argument(
        "--score-cut",
        type=float,
    ...
    if not args.skip_build:
        print("=" * 70)
        print("STEP 1: MLP HELIX SEGMENT MATCHING")
        print("=" * 70)
        print()

        # Load model
        mlp_config = config.get("mlp", {})
        model_path = mlp_config.get("model_path", "saved_models/mlp_segment_matcher.pt")
        if not Path(model_path).is_absolute():
            model_path = str(PIPELINE_ROOT / model_path)

        if not Path(model_path).exists():
            raise FileNotFoundError(
                f"Trained MLP not found at: {model_path}\n"
                f"Run train_mlp_segment_matcher.py first."
            )

        print(f"Loading MLP from: {model_path}")
        model = load_mlp_model(model_path, mlp_config, device=device)
        print()

        run_mlp_segment_matching(
            args.dataset,
            config,
            model,
            device,
